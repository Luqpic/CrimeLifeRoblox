...
(`SpraypaintService.awardForVictim`) directly rather than the production `Eliminated` wire; that wire
...
## Summary
Phase 2 ships Spraypaint: a second currency, earned only from NPC kills (Thug/Criminal +2, Police +5) and
...
The shop's skin row gained a coin badge on unowned Spraypaint chips, a buy strip (PRICE / BUY / CANCEL)
...
attribute that `LevelingService` writes and the HUD already reads. `SpraypaintService` adds two more
listeners onto signals the game already fires — `SpraypaintRunner`, a one-line `Script`, is the only thing
that connects them, and nothing else requires `SpraypaintService` — so the earn path is additive rather than
...
`Constants.spraypaintForFaction` returns `0` for anything that is not a recognised NPC faction string,
which includes `nil`. Without this, two players could stand in front of each other and trade kills to
print currency; `SpraypaintConstants_Test` and `SpraypaintService_Test` both lock the property (a `nil`,
...
pass measured:
- `SpraypaintService.onEliminated` (the guard actually wired to `Eliminated`) is split from
  `SpraypaintService.awardForVictim` (the reward rule). A `Player` cannot be constructed with
  ...
  used that seam deliberately (see Notes) to get a clean kill-Spraypaint reading without a confound from
  `LevelingService`, which shares the same `Eliminated` event.
- `CosmeticsService.ownsPalette`'s Spraypaint branch, and the client's copy in `SkinRowController`, both
  gate on `palette.source == "Spraypaint"`, not on `palette.price` being truthy. The two are equivalent for
  today's catalogue, but a future Spraypaint palette shipped without a price would fall through a
...
repo's mirrors under `FPSSystem/Spraypaint/`, checked live):
- `ReplicatedStorage.Spraypaint.Constants` (ModuleScript, 1722 bytes) — `ATTRIBUTE = "Spraypaint"`,
  `KILL_SPRAYPAINT = { Criminal = 2, Police = 5 }`, `LEVEL_SPRAYPAINT = 25`, `spraypaintForFaction`,
  `ownedAttributeFor`.
- `ReplicatedStorage.Spraypaint.Remotes.BuySkinRequest` (RemoteEvent) — the one client-to-server purchase
  entry point, taking `(paletteKey, weaponName)`.
- `ServerScriptService.Spraypaint.Scripts.SpraypaintService` (ModuleScript, 8127 bytes) — `balanceOf`, `grant`,
  `spend`, `onEliminated`/`awardForVictim`, `onLevelChanged`, `handleBuyRequest`, `start`.
- `ServerScriptService.Spraypaint.Scripts.SpraypaintRunner` (Script, 179 bytes) — the sole caller of
  `SpraypaintService.start()`; requiring the module alone connects nothing.
- `StarterPlayer.StarterPlayerScripts.SpraypaintHud` (LocalScript, 1084 bytes) — the Spraypaint gain
  notification.
- `ServerStorage.UnitTest.Cases.SpraypaintConstants_Test` (8 cases), `SpraypaintService_Test` (20 cases),
  `SpraypaintTier_Test` (8 cases) — the three new suites re-run below.
...
- `ReplicatedStorage.Cosmetics.Palettes` (4412 bytes) — extended from 8 to 12 entries: `Cobalt` (250),
  `Verdigris` (350), `Ember` (500), `RoseGold` (750), all `source = "Spraypaint"`. `Palette` type gained an
  optional `price: number?`, present only on Spraypaint entries. `Palettes.priceOf` added.
- `ServerScriptService.Cosmetics.Scripts.CosmeticsService` (4782 bytes) — `ownsPalette` gained the
  Spraypaint branch described in Cause, gated on `source`, failing closed.
- `StarterPlayer.StarterPlayerScripts.SkinRowController` (12718 bytes) — a `Coin` badge on each Spraypaint
  ...
  own review caught: without it, the coin badge stayed hidden on unowned Spraypaint chips until the first
  ownership change).
...
1. **Kills award Spraypaint — PASS.** Baseline `Level=4 Spraypaint=75`. Called `SpraypaintService.awardForVictim`
   directly (methodology explained in Notes) against the real player `Instance` with a freshly created
   `Model`+`Humanoid` victim: `Faction="Criminal"` (Thug's own template value) paid **Spraypaint 75 → 77
   (+2)**; `Faction="Police"` paid **77 → 82 (+5)**. `Level` read `4` both before and after — pinned,
   no confound.
2. **Level-ups award Spraypaint — PASS.** `player:SetAttribute("Level", 2)` from `1`, then
   `player:GetAttribute("Spraypaint")` immediately read **`0`** (unchanged) and only after one `task.wait()`
   read **`25`** — this place's attribute-changed signals fire deferred, not synchronously (see Notes;
   this is the mechanism behind a trap already on record from Task 2). **Spraypaint 0 → 25 (+25), exact.**
3. **Joining awards nothing — PASS.** Read at the very first query after entering Play, before any test
   action: **`Level=1 Spraypaint=0`**. Independently reconfirmed at row 13's fresh session: `Level=1
   Spraypaint=0` again.
4. **A multi-level jump pays per level — PASS.** From `Level=2 Spraypaint=25`, set `Level` to `4` (a
   two-level jump) and `task.wait()`. Result: **`Level=4 Spraypaint=75`, delta +50** — exactly `2 ×
   LEVEL_SPRAYPAINT(25)`, not a flat 25 for "a" level-up.
5. **PvP kills award nothing — PASS.** Called `awardForVictim(player, player.Character.Humanoid)` —
   ...
   attribute, exactly the real PvP shape rather than a constructed stand-in. **Spraypaint 82 → 82 (+0).**
6. **Purchase deducts and grants — PASS.** Seeded `Spraypaint=1000` server-side, fired the real
   `BuySkinRequest:FireServer("Cobalt", "AK47")` from the **Client** datamodel. Result: **`Spraypaint 1000 →
   750` (exact −250), `SkinOwned_Cobalt=true`, `EquippedSkin_AK47="Cobalt"`.**
7. **Insufficient funds refused — PASS.** Seeded `Spraypaint=100`, fired
   `BuySkinRequest:FireServer("Verdigris", "AK47")` (price 350). Result: **`Spraypaint` stayed `100`,
   `SkinOwned_Verdigris` stayed `nil`** — refused, nothing changed.
8. **Double purchase refused — PASS.** Seeded `Spraypaint=1000`, fired
   `BuySkinRequest:FireServer("Ember", "AK47")` **twice**, back to back with no yield between the two
   `FireServer` calls. Result: **`Spraypaint 1000 → 500`** — fell exactly once at the 500 price, not to 0 —
   and `SkinOwned_Ember=true`.
9. **A Vip key cannot be bought with Spraypaint — PASS.** Seeded `Spraypaint=99999` (ruling out "insufficient
   funds" as the refusal reason) with `GamepassOwned_Vip=false` confirmed. Fired
   `BuySkinRequest:FireServer("Gold", "AK47")`. Result: **`Spraypaint` stayed `99999`,
   `GamepassOwned_Vip` stayed `false`, `EquippedSkin_AK47` stayed `"Ember"`** — refused by the `source
   == "Spraypaint"` gate, not by balance.
10. **Forged request refused — PASS.** Seeded `Spraypaint=0`. Fired both
    `BuySkinRequest:FireServer("RoseGold", "AK47")` (an unowned Spraypaint palette) and
    `BuySkinRequest:FireServer("Gold", "AK47")` (the Vip key) at zero balance. Result: **`Spraypaint` stayed
...
11. **Phase 1 still passes — PASS.** Run in Play (fresh `require` cache; see Notes for why Edit mode is
    ...
    exactly matching Phase 1's own numbers, unaffected by the four new Spraypaint palettes or the
    `ownsPalette` Spraypaint branch.
...
13. **Nothing persists — PASS.** End-of-first-session state (after rows 1-12 had written test balances
    and ownership): `Level=4 Spraypaint=0 SkinOwned_Cobalt=true SkinOwned_Verdigris=nil SkinOwned_Ember=true
    SkinOwned_RoseGold=nil EquippedSkin_AK47="Ember"`. Stopped Play, started a fresh Play session. Same
    player, freshly joined: **`Level=1 Spraypaint=0 SkinOwned_Cobalt=nil SkinOwned_Verdigris=nil
...
`ServerScriptService.Weapons.Scripts.WeaponUpgradeService`'s `Source` contains `"Spraypaint"` or
`"Cosmetics"`.
...
warned it would — that file was touched by **Phase 1**, before Spraypaint existed, since `96e40fd` predates
...
`FPSSystem/Spraypaint/` (`Constants.luau`, `SpraypaintService.luau`, `SpraypaintRunner.luau`) plus their two test
mirrors and `SpraypaintHud.luau`, `Palettes.luau`, `CosmeticsService.luau`, `SkinRowController.luau`, and
...
- `SpraypaintConstants_Test`: **8/8**
- `SpraypaintService_Test`: **20/20**
- `SpraypaintTier_Test`: **8/8**
- `Palettes_Test`: **10/10**
...
`Eliminated` BindableEvent is not Spraypaint's alone — `LevelingService` also listens to it and grants XP
on every kill, independent of faction, and that XP can cross a level threshold and trigger its own
Spraypaint grant (`+25`, via `onLevelChanged`). Firing `Eliminated:Fire(player, victimHumanoid)` for a kill
...
called `SpraypaintService.awardForVictim` (and, for row 5's real-Faction-less case, the exact same function)
**directly** — the identical function `SpraypaintService.start()` wires to `Eliminated` via `onEliminated`,
confirmed by reading `SpraypaintService`'s source — against the **real, live player `Instance`** (not a
...
`player:SetAttribute("Level", 2)` and reading `player:GetAttribute("Spraypaint")` in the very next line,
same Luau thread, no yield, read the **unchanged** balance (`0`); only after a `task.wait()` did the
listener wired via `player:GetAttributeChangedSignal("Level")` in `SpraypaintService.start()` run and
produce `25`. This means a script that writes `Level` and reads `Spraypaint` back-to-back with no yield will
...
found that `run("Palettes")` originally swept up `SpraypaintPalettes_Test` too (returning 18, not Phase 1's
10) because `RunUnitTest`'s filter is a plain, unanchored `string.find`, and `"SpraypaintPalettes_Test"`
contains `"Palettes_Test"` as a substring. The fix was a rename (`SpraypaintPalettes_Test` →
`SpraypaintTier_Test`), not a filter change — `RunUnitTest` itself is unmodified and the underlying design
...
Spraypaint-earning PvP kill. It was not: a level-up happened to land inside a ~0.2s wait window between two
separate measurements, coincidentally worth the same `+25` `LEVEL_SPRAYPAINT` figure as the number being
...
and compared to this repo's copy under `FPSSystem/`: `Constants.luau` 1722, `SpraypaintService.luau` 8127,
`SpraypaintRunner.luau` 179, `SpraypaintHud.luau` 1084, `Palettes.luau` 4412, `CosmeticsService.luau` 4782,
...
- **Confirmed live, with numbers, this pass:** all thirteen verification rows, the damage-path-clean
  ...
  `SpraypaintConstants` 8/8, `SpraypaintService` 20/20, `SpraypaintTier` 8/8, `Palettes` 10/10, `SkinApplier` 14/14,
  `CosmeticsOwnership` 11/11). Carve-out: rows 1 and 5 measured the earn rule directly
  (`SpraypaintService.awardForVictim`), not the production wire. The `Eliminated -> onEliminated ->
